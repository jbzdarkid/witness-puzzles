...
function validate(puzzle, quick=false) {
  console.log('Validating', puzzle)
  puzzle.valid = true // Assume valid until we find an invalid element

  // Compute failures which will not appear during region checks
  ...
  for (var x=0; x<puzzle.width; x++) {
    for (var y=0; y<puzzle.height; y++) {
      ...
      if (cell.line > window.LINE_NONE) {
        if (cell.dot > 0 && cell.line != cell.dot) {
          console.log(cell.dot + ' color dot at', x, y, 'is covered by', cell.line, 'color line')
          puzzle.valid = false
          if (quick) return
        }
  ...
  for (var region of regions) {
    ...
    if (regionData == undefined) {
      ...
      if (quick && !regionData.valid()) return
    }
...
function _regionCheckNegations2(puzzle, region, negationSymbols, invalidElements, index=0, index2=0) {
  if (index2 >= negationSymbols.length) {
    ...
    return _regionCheck(puzzle, region, true)
  }
...
function regionCheckNegations(puzzle, region, quick) {
  ...
  // Else, get a list of elements that are currently invalid (before any negations are applied)
  // Must not be quick validation, as we need a complete list of invalid elements.
  var regionData = _regionCheck(puzzle, region, false)
